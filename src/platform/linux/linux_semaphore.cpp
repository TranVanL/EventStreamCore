...
[[noreturn]] void throwSemaphoreError(
    int error,
...
timespec realtimeDeadline(std::chrono::nanoseconds timeout) {
    if (timeout < std::chrono::nanoseconds::zero()) {
        throw std::invalid_argument(
            "LinuxSemaphoreBackend timeout cannot be negative");
    }
...
LinuxSemaphoreBackend::LinuxSemaphoreBackend(
...
LinuxSemaphoreBackend::~LinuxSemaphoreBackend() noexcept {
    if (initialized_) {
        (void)sem_destroy(&semaphore_);
    }
}

void LinuxSemaphoreBackend::wait() {
    for (;;) {
...
bool LinuxSemaphoreBackend::try_wait() {
...
bool LinuxSemaphoreBackend::wait_for(
...
void LinuxSemaphoreBackend::post() {
    if (sem_post(&semaphore_) != 0) {
        throwSemaphoreError(errno, "sem_post");
    }
}

unsigned LinuxSemaphoreBackend::value() const {
    int value = 0;
    if (sem_getvalue(
            const_cast<sem_t*>(&semaphore_),
            &value) != 0) {
...
LinuxSemaphoreBackend::NativeHandle
LinuxSemaphoreBackend::native_handle() noexcept {
    return &semaphore_;
}
